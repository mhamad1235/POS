...
<body  style="height:200%">
    ...
    <a style="color: black" href="#"> <svg data-bs-toggle="offcanvas" data-bs-target="#offcanvasTop" aria-controls="offcanvasTop"data-bs-toggle="offcanvas" data-bs-target="#offcanvasTop" aria-controls="offcanvasTop" style="position: relative;left:6%;" xmlns="http://www.w3.org/2000/svg" width="45" height="45" fill="currentColor" class="bi bi-plus-circle-fill" viewBox="0 0 16 16">
      ...
      <h3 style="position:relative;left:10%;">Customer</h3>
        <div style="height: 50%" class="offcanvas offcanvas-top" tabindex="-1" id="offcanvasTop" aria-labelledby="offcanvasTopLabel">
          <div class="offcanvas-header">
            <h5 class="offcanvas-title" id="offcanvasLabel">Add Customer</h5>
            <button  type="button" class="btn btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
          </div>
       
          <form action="{{route('customeradd')}}" method="post">
        ...
        <table style="width: 90%;position:relative;left:10%;" class="table table-striped text-center">
         ...
         @foreach ($customer as $customer)
             <tr>
                ...
                <td><button name="dept" value="{{$customer->name}}"  class="btn btn-secondary">Dept</button></td>
              </form>
              <td><a href="/delete/{{$customer->id}}"><button class="btn btn-danger">Delete</button></a> <a href="/update/{{$customer->id}}"><button class="btn btn-success" >Update</button></a></td>
              </tr>
         @endforeach
                               ...
                               <div>
                                   <table class="table table-bordered">
                                       ...
                                       <tbody>
                                            ...
                                            @for ($i = 0; $i < count($item['name_product']); $i++)
                                                
                                            <tr>
                                                <th>{{$item['name_product'][$i]}}</th>
                                                <th>{{$item['quantity'][$i]}}</th>
                                                <th>{{$item['nrx_tak'][$i]}} $ </th>
                                                <th>{{($item['nrx_tak'][$i])*($item['quantity'][$i])}} $ </th>
                                            </tr>
                                            @endfor
                                           <tr>
